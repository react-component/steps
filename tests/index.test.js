...
import { ENTER_KEY_CODE, SPACE_KEY_CODE } from '../src/constants';
...
describe('Steps', () => {
  ...
  describe('onChange', () => {
    ...
    const step = wrapper.find('.rc-steps-item-container').at(1);

    beforeEach(() => {
      onChange.mockClear();
    });

    it('call onChange when the step is clicked', () => {
      step.simulate('click');
      expect(onChange).toBeCalledWith(1);
    });

    it('call onChange when the step has the enter pressed', () => {
      step.simulate('keyup', { keyCode: ENTER_KEY_CODE });
      expect(onChange).toBeCalledWith(1);
    });

    it('call onChange when the step has the space bar pressed', () => {
      step.simulate('keyup', { keyCode: SPACE_KEY_CODE });
      expect(onChange).toBeCalledWith(1);
    });
  });
  ...
  describe('disabled', () => {
    ...
    const step = wrapper.find('.rc-steps-item-container').at(2);

    it(`don't call onChange when the step is clicked`, () => {
      step.simulate('click');
      expect(onChange).not.toBeCalled();
    });

    it(`don't call onChange when the step has the enter pressed`, () => {
      step.simulate('keyup', { keyCode: ENTER_KEY_CODE });
      expect(onChange).not.toBeCalled();
    });

    it(`don't call onChange when the step has the space bar pressed`, () => {
      step.simulate('keyup', { keyCode: SPACE_KEY_CODE });
      expect(onChange).not.toBeCalled();
    });
  });
